...
def visualize_trajectory_2d(pose, landmark_b=None, pose_2 = None, landmark_c=None, show_ori=False, show_landmark_b=False, show_pose2 = None,show_landmark_c=False,):
  '''
  ...
  n_pose = pose.shape[2]
  ax.plot(pose[0,3,:],pose[1,3,:],color='red',label='a_path',linewidth=2.5)
  ax.scatter(pose[0,3,0],pose[1,3,0],color='red', marker='s',label="start")
  ax.scatter(pose[0,3,-1],pose[1,3,-1],color='red', marker='*',label="end")
  if show_ori:
  ...
  if show_landmark_b:
    ax.scatter(landmark_b[0, :], landmark_b[1, :],color='lime', marker = 'o', label="b_landmark")
  if show_pose2:
    ax.plot(pose_2[0, 3, :], pose_2[1, 3, :], color='blue',label='c_path',linewidth=2.5)
    ax.scatter(pose_2[0, 3, 0], pose_2[1, 3, 0], color='blue',marker='s', label="start")
    ax.scatter(pose_2[0, 3, -1], pose_2[1, 3, -1], color='blue', marker='*', label="end")
  if show_landmark_c:
    ax.scatter(landmark_c[0, :], landmark_c[1, :], color='brown', label="c_landmark")
  ax.set_xlabel('x')
  ax.set_ylabel('y')
  ax.axis('equal')
  ax.grid(False)
